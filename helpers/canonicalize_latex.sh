...
perl -0777 -pe "s/\\\\begin\{quotation\}(.*?)\\\\end\{quotation\}/\\\\begin{quote}% <attr role=quote>\n\\\\itshape \1\\\\end{quote}/gs" | \
...
# chapter title with TOC text
perl -0777 -pe "s/\\\\chapter[[]([^]]+)[]]\{.*\}/\\\\chapter{\1}/gs" | \
...
perl -0777 -pe "s/\\\\textls\{(.*?)\}/\1/gs" | \
perl -0777 -pe "s/\\\\question\{(.*?)\}/\\\\emph{\1}/gs" | \
perl -0777 -pe "s/\\\\answer\{(.*?)\}/\\\\emph{\1}/gs" | \
...
perl -0777 -pe "s/\\\\cite\{(.*?)\}/\\\\footnote{FIXME:cite \1}/gs" | \
